...
★ 真實資料上的結果是全部 0 筆交易——這是查過的，不是還沒查的已知限制：
  用真實 150 檔股票資料跑過診斷，四個進場條件（魚池+營收動能+價量突破）
  同時成立、且大盤 regime 也是綠燈的候選只有 56 筆（3 年裡不算少見，但
  已經很稀少），但這 56 筆全部被系統既有的「單一產業曝險上限 10%」全域
  風控擋掉，一筆都沒有真正成交。原因：營收成長 + 價量突破這組條件很容易
  在同一個產業裡「群聚」發生（景氣循環股常常整個類股一起噴），而 2% 固定
  風險的部位規模模型算出來的單筆部位金額，對這些股票來說已經逼近或超過
  10% 產業曝險上限，導致同產業裡第一筆之後的候選、甚至第一筆本身都可能
  直接被擋——這是系統既有全域風控（跟策略 C 等其他策略共用的機制）跟這組
  特定訊號的產業群聚特性交互作用的結果，不是這個腳本本身的 bug（用
  scripts/debug_breakout_combo.py 這個一次性診斷腳本查過、已刪除）。想看
  這個訊號本身有沒有 edge，需要另外放寬或關掉產業曝險上限重測，這裡沒有
  這樣做，因為那樣會失去跟其他策略之間可比較的統一風控基準。

...
這裡直接測試持股檔數這一個維度，把範圍拉得更極端：3/5 檔（非常集中，
賭少數幾檔真正的動量龍頭）一路到 50 檔（比原本測過的 30 檔更分散），
看報酬曲線的形狀有沒有隨著集中度提高而更貼近 QQQ——集中度越高，理論上
越可能捕捉到「少數贏家帶動指數」的效果，但代價通常是波動度、換手時的
市場衝擊成本、單一持股權重過高的風險都會上升。
...
def main() -> None:
    ...
    print(
        "\n（誠實揭露：top_n 越小，單一持股權重越高——3 檔等權重意味著單檔佔投資組合\n"
        "1/3，真實下單時的市場衝擊成本、個股風險集中度都會遠高於這裡的回測假設\n"
        "（複委託固定手續費模型沒有模擬大額單一持股的滑價），數字要打折扣看待；\n"
        "3/5 檔這種極端集中的組合，樣本內外的表現對「哪一輪剛好選中/沒選中對的\n"
        "1~2 檔龍頭股」高度敏感，運氣成分比 20/30/50 檔的組合大得多）"
    )

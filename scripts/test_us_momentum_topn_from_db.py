...
def _run(us_prices: pd.DataFrame, base_cfg, top_n: int, start_date, end_date, membership) -> dict:
...
def _print_period(label: str, us_prices, base_cfg, start_date, end_date, qqq_bench: dict, membership) -> None:
    ...
    for top_n in TOP_N_GRID:
        m = _run(us_prices, base_cfg, top_n, start_date, end_date, membership)
...
def main() -> None:
    ...
    _print_period("樣本外（挑持股檔數網格時完全沒看過的更早期間，2018-09-20 ~ 2023-09-18）", us_prices, base_cfg, OOS_START, OOS_END, QQQ_OOS, membership)

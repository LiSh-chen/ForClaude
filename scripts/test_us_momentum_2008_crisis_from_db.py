...
2026-09-20 補充：20 年回填跑完後，匯出成 repo 快照這一步一度失敗——
data/us_prices_snapshot.parquet 膨脹到 101.2 MB，超過 GitHub 單檔 100 MB
push 上限（pre-receive hook 擋下）。改成直連資料庫繞過這個限制後，使用者
明確要求不要繞過快照架構，改成把快照本身拆成多檔（見
tw_quant/data_snapshot.py 檔頭「us_prices 分檔」說明：第一份沿用原本檔名，
超過門檻的部分依序寫成 .part2、.part3……），維持「所有策略腳本一律讀

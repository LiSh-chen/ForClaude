...
美股真正不同的部分），差異只有兩處：
  ...
  - sizing.lot_size = 1：美股沒有台股「1 張 = 1000 股」的整股倍數限制，
    可以買賣任意股數。
  - costs：改用 tw_quant/us_costs.py 的美股費率（沒有證交稅、零手續費、
    $0.01 tick）。
...
from tw_quant.us_costs import US_FEE_RATE, US_TAX_RATE


def build_us_config() -> StrategyConfig:
    cfg = StrategyConfig()
    cfg.sizing.lot_size = 1
    cfg.costs = CostConfig(tax_rate=US_TAX_RATE, fee_rate=US_FEE_RATE, exit_slippage_ticks=2)
    return cfg
